...
def IsVonNeumannAlgebra (M : Set HSTPOp) : Prop :=
  ...
  (∀ S T : HSTPOp, S ∈ M → T ∈ M →
    CATEPTMain.Quantum.HSTP.Strong_Operator_Topology.hstpOpComp S T ∈ M) ∧
  -- 4. SOT-closed:
  (∀ Tseq : ℕ → HSTPOp, (∀ n, Tseq n ∈ M) →
    ∀ T : HSTPOp, CATEPTMain.Quantum.HSTP.Strong_Operator_Topology.HSTPStrongConv Tseq T →
    T ∈ M)

-- ── Commutant ─────────────────────────────────────────────────────────────────
def commutant (M : Set HSTPOp) : Set HSTPOp :=
  { S | ∀ T ∈ M,
    CATEPTMain.Quantum.HSTP.Strong_Operator_Topology.hstpOpComp S T =
    CATEPTMain.Quantum.HSTP.Strong_Operator_Topology.hstpOpComp T S }
