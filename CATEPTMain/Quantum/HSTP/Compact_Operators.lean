...
open CATEPTMain.Quantum.HSTP.Eigenvalues (IsHSTPCompact)
...
private axiom hs_compact_law (T : CBOOp) (hHS : CATEPTMain.Quantum.HSTP.HS2Ell2.IsHilbertSchmidt T) :
    ∃ T' : HSTPOp, IsHSTPCompact T'

theorem hs_compact (T : CBOOp) (hHS : CATEPTMain.Quantum.HSTP.HS2Ell2.IsHilbertSchmidt T) :
...
theorem compact_selfadj_has_eigenvalue (T : HSTPOp)
    ...
    (hSA : hstpOpAdj T = T) :
    ∃ (ev : ℝ), CATEPTMain.Quantum.HSTP.Eigenvalues.IsHSTPEigenvalue T ev := by
  obtain ⟨evs, vs, hEig, _, _⟩ :=
    CATEPTMain.Quantum.HSTP.Eigenvalues.compact_selfadj_eigenbasis T hCompact hSA
  exact ⟨evs 0, hEig 0⟩
