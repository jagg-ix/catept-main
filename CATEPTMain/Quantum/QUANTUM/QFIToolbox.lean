...
/-- Bipartite entanglement entropy is non-negative. -/
theorem bipartiteEntanglementEntropy_nonneg (nA nB : ℕ) (ρ : DensityMatrix (nA * nB)) :
    0 ≤ bipartiteEntanglementEntropy nA nB ρ := by
  unfold bipartiteEntanglementEntropy
  exact vonNeumannEntropy_nonneg nA
    ⟨@partialTrace nA nB ρ.mat,
      partialTrace_hermitian ρ.mat ρ.herm,
      partialTrace_psd ρ.mat ρ.psd,
      by simpa [ρ.tr1] using (partialTrace_trace (ρ := ρ))⟩
