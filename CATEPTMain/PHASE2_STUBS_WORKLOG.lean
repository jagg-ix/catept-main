...
3. Update the barrel `CATEPTMain/AFPBridge.lean`:
   - Remove `import CATEPTMain.MOD.MODPrelude`
   + Add (once) `import CATEPTMain.Stubs`
...
### Notes
  - Keep GYRPrelude's `import CATEPTMain.Framework.AFPBridgeFramework`
    at the top of Stubs.lean (only once).
  - Namespace aliases (`alias`) are NOT needed — the namespace names stay
    identical (e.g., `CATEPTMain.GYR` still exists, just in Stubs.lean).
