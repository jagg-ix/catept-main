  ...
  (List.range n).foldl (fun comps μ =>
    (List.range n).foldl (fun comps ν =>
      (List.range n).foldl (fun comps λ_ =>
        let val := simplify
          (.sub (.mul (x.get! μ) (matGet tCon ν λ_))
                (.mul (x.get! ν) (matGet tCon μ λ_)))
        comps.set! (μ*n*n + ν*n + λ_) val
      ) comps
    ) comps
        ...
        let val : Expr := match i1, i2, i3 with
          ...
          | true,  true,  true  =>
              sumN n (fun a => sumN n (fun b => sumN n (fun c =>
                simplify (.mul (.mul (.mul (matGet gCov i a) (matGet gCov j b))
...
def ofStressEnergy (st : StressEnergyTensor) (x : Array Expr := #[])
  ...
  let x' := if x.isEmpty then Array.ofFn (fun i => .var s!"X{i.val}") else x
