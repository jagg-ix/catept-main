...
namespace CATEPTMain.QuantumGravity.QPICore
...
theorem hadamardFluctuationGate_is_gate (n : ℕ) :
  ...
  CATEPTMain.QuantumOps.IsabelleMarresDirac.Quantum.iter_tensor_H_n_is_gate n
...
def quantumGeometryState (n : ℕ) : NQubitSpace n :=
  hadamardFluctuationGate n *
    CATEPTMain.QuantumOps.IsabelleMarresDirac.Quantum.ket_zero_n n
...
theorem quantumGeometryState_is_state (n : ℕ) :
  ...
  set v := CATEPTMain.QuantumOps.IsabelleMarresDirac.Quantum.ket_zero_n n
  have hU : U ∈ Matrix.unitaryGroup (Fin (2 ^ n)) ℂ := hadamardFluctuationGate_is_gate n
  have hv : ∑ i : Fin (2 ^ n), Complex.normSq (v i 0) = 1 :=
    CATEPTMain.QuantumOps.IsabelleMarresDirac.Quantum.all_zero_state n
...
end CATEPTMain.QuantumGravity.QPICore
