...
namespace CATEPTMain.AFPBridge.LSI.Theories.Lebesgue_Stieltjes_Integral
...
theorem lsi_integral_linear (F : ℝ → ℝ) (hF : Monotone F)
    ...
    c * lsiIntegral F f + lsiIntegral F g := by
  sorry -- phase2_exact: MeasureTheory.integral_add + integral_smul

theorem lsi_integral_nonneg (F : ℝ → ℝ) (hF : Monotone F)
    (f : ℝ → ℝ) (hf : ∀ x, 0 ≤ f x)
    (hfInt : MeasureTheory.Integrable f (lsiMeasure F)) :
    0 ≤ lsiIntegral F f := by
  sorry -- phase2_exact: MeasureTheory.integral_nonneg

theorem lsi_integral_mono (F : ℝ → ℝ) (hF : Monotone F)
    (f g : ℝ → ℝ) (hfg : ∀ x, f x ≤ g x)
    (hf : MeasureTheory.Integrable f (lsiMeasure F))
    (hg : MeasureTheory.Integrable g (lsiMeasure F)) :
    lsiIntegral F f ≤ lsiIntegral F g := by
  sorry -- phase2_exact: MeasureTheory.integral_mono
...
theorem lsi_from_integral_function (f g : ℝ → ℝ)
    ...
    lsiIntegral F g =
    ∫ x, g x * f x ∂MeasureTheory.volume := by
  sorry -- phase2_exact: withDensity_eq + change_of_variables
...
theorem lsi_fubini (F G : ℝ → ℝ) (hF : Monotone F) (hG : Monotone G) (a b : ℝ) :
    ...
    ∫ y in Set.Ioc a b, lsiIntegralOn F (fun _ => 1) a y ∂(lsiMeasure G) := by
  sorry -- phase2_exact: MeasureTheory.lintegral_prod (Fubini-Tonelli)
...
theorem lsi_indicator_Ioc (F : ℝ → ℝ) (hF : Monotone F) (a b : ℝ) (h : a ≤ b) :
    ...
    F b - F a := by
  sorry -- phase2_exact: integral_indicator + lsiMeasure_Ioc
...
theorem lsi_continuous_integrable (F g : ℝ → ℝ) (hF : Monotone F) (a b : ℝ)
    ...
    MeasureTheory.IntegrableOn g (Set.Ioc a b) (lsiMeasure F) := by
  sorry -- phase2_exact: MeasureTheory.ContinuousOn.integrableOn_Ioc

end CATEPTMain.AFPBridge.LSI.Theories.Lebesgue_Stieltjes_Integral
