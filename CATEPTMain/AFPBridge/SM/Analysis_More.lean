...
namespace CATEPTMain.AFPBridge.SM.Analysis_More

open CATEPTMain.AFPBridge.SM

-- ── Inverse function theorem ──────────────────────────────────────────────────
private axiom inverse_function_law (n : ℕ) (f : EuclideanSpace ℝ (Fin n) → EuclideanSpace ℝ (Fin n))
    ...
    ∃ g : EuclideanSpace ℝ (Fin n) → EuclideanSpace ℝ (Fin n),
      ContDiff ℝ ⊤ g ∧ Set.EqOn (g ∘ f) id U
...
theorem inverse_function (n : ℕ) (f : EuclideanSpace ℝ (Fin n) → EuclideanSpace ℝ (Fin n))
    ...
    ∃ g : EuclideanSpace ℝ (Fin n) → EuclideanSpace ℝ (Fin n),
      ContDiff ℝ ⊤ g ∧ Set.EqOn (g ∘ f) id U :=
  inverse_function_law n f hf x hInj

-- ── Smooth bump on ℝⁿ ────────────────────────────────────────────────────────
-- ∃ f : ℝⁿ → ℝ, Cᵢ∞, f = 1 near 0, supp f ⊆ ball 1.
private axiom smooth_bump_exists_law (n : ℕ) :
    ∃ f : EuclideanSpace ℝ (Fin n) → ℝ,
      ContDiff ℝ ⊤ f ∧
      (∀ x, 0 ≤ f x) ∧
      (∀ x, ‖x‖ ≤ 1/2 → f x = 1) ∧
      Function.support f ⊆ Metric.ball 0 1
...
theorem smooth_bump_exists (n : ℕ) :
    ∃ f : EuclideanSpace ℝ (Fin n) → ℝ,
      ...
      (∀ x, 0 ≤ f x) ∧
      (∀ x, ‖x‖ ≤ 1/2 → f x = 1) ∧
      Function.support f ⊆ Metric.ball 0 1 := smooth_bump_exists_law n

-- ── Locally Lipschitz → continuous ────────────────────────────────────────────
private axiom locallyLipschitz_cont_law {E F : Type*}
    [NormedAddCommGroup E] [NormedSpace ℝ E] [NormedAddCommGroup F] [NormedSpace ℝ F]
    (f : E → F) (hLip : ∀ x : E, ∃ K : NNReal, ∃ U : Set E, IsOpen U ∧ x ∈ U ∧
    LipschitzOnWith K f U) : Continuous f

theorem locallyLipschitz_cont {E F : Type*}
    [NormedAddCommGroup E] [NormedSpace ℝ E] [NormedAddCommGroup F] [NormedSpace ℝ F]
    (f : E → F) (hLip : ∀ x : E, ∃ K : NNReal, ∃ U : Set E, IsOpen U ∧ x ∈ U ∧
    LipschitzOnWith K f U) : Continuous f := locallyLipschitz_cont_law f hLip

end CATEPTMain.AFPBridge.SM.Analysis_More
