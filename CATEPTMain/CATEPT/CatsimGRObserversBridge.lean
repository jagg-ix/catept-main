...
/-- Effective-g₀₀ formula for a rotating observer with angular velocity Ω:
    g_eff = g_tt + 2 g_tφ Ω + g_φφ Ω² . -/
def rotating_observer_g_eff (g_tt g_tphi g_phiphi Ω : ℝ) : ℝ :=
...
def rotating_observer_dtau_dt
  ...
  Real.sqrt (-(rotating_observer_g_eff g_tt g_tphi g_phiphi Ω))
