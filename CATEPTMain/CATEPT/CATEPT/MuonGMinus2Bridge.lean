...
backreaction on the muon loop, producing a correction
δa_μ = entropicCoupling·α where entropicCoupling encodes the entropic
coupling strength. This backreaction picture
...
/-- Entropic correction to g-2: `δa_μ = entropicCoupling · α`, where
    `entropicCoupling` encodes the CAT/EPT backreaction strength from `S_I`.
    This disambiguates the muon coefficient from hyperbolic-sector `κ`,
    thermal `k_B`, and the speed-of-light constant `c`. -/
def entropic_g2_correction (entropicCoupling α : ℝ) : ℝ := entropicCoupling * α

theorem entropic_g2_correction_nonneg (entropicCoupling α : ℝ)
    (hEntCoupling : 0 ≤ entropicCoupling) (hα : 0 ≤ α) :
    0 ≤ entropic_g2_correction entropicCoupling α :=
  mul_nonneg hEntCoupling hα

/-- The correction vanishes when `entropicCoupling = 0`
    (decoupled regime). -/
theorem entropic_g2_correction_decoupled (α : ℝ) :
    entropic_g2_correction 0 α = 0 := by
  unfold entropic_g2_correction; ring

/-- Monotonicity: larger entropic coupling gives a larger correction. -/
theorem entropic_correction_monotone (entropicCoupling₁ entropicCoupling₂ α : ℝ)
    (hα : 0 < α) (hEntCoupling : entropicCoupling₁ < entropicCoupling₂) :
    entropic_g2_correction entropicCoupling₁ α < entropic_g2_correction entropicCoupling₂ α := by
  unfold entropic_g2_correction
  exact mul_lt_mul_of_pos_right hEntCoupling hα

/-! ## Backreaction Scale -/

/-- Entropic backreaction scale:
    `backreactionScale = entropicCoupling · α · ℏ · c / m²`.
    This is a loop-level correction scale, not a replacement Newton constant. -/
def entropicBackreactionScale (entropicCoupling α ℏ c m : ℝ) : ℝ :=
  entropicCoupling * α * ℏ * c / m ^ 2

theorem entropicBackreactionScale_nonneg (entropicCoupling α ℏ c m : ℝ)
    (hEntCoupling : 0 ≤ entropicCoupling) (hα : 0 ≤ α) (hh : 0 ≤ ℏ) (hc : 0 ≤ c) :
    0 ≤ entropicBackreactionScale entropicCoupling α ℏ c m := by
  unfold entropicBackreactionScale
  apply div_nonneg
  · exact mul_nonneg (mul_nonneg (mul_nonneg hEntCoupling hα) hh) hc
  · exact sq_nonneg m
