...
backreaction** on the muon loop:
  ...
  δa_μ = (backreactionScale ℏ / c⁵)^{1/2} · λ · f(ω_muon, θ_spin, φ̇)

where backreactionScale = entropicCoupling · α · ℏ · c / m² is the
entropic loop backreaction scale from entropic curvature. To first order, this reduces to
δa_μ = entropicCoupling · α.
...
example (entropicCoupling α : ℝ) (hEntCoupling : 0 ≤ entropicCoupling) (hα : 0 ≤ α) :
    0 ≤ entropic_g2_correction entropicCoupling α :=
  entropic_g2_correction_nonneg entropicCoupling α hEntCoupling hα

-- In the decoupled regime (entropicCoupling=0), the correction vanishes
example (α : ℝ) :
    entropic_g2_correction 0 α = 0 :=
  entropic_g2_correction_decoupled α

-- Larger entropic coupling → larger correction (monotonicity)
example (entropicCoupling₁ entropicCoupling₂ α : ℝ)
    (hα : 0 < α) (hEntCoupling : entropicCoupling₁ < entropicCoupling₂) :
    entropic_g2_correction entropicCoupling₁ α < entropic_g2_correction entropicCoupling₂ α :=
  entropic_correction_monotone entropicCoupling₁ entropicCoupling₂ α hα hEntCoupling
...
-- Entropic backreaction scale is non-negative
example (entropicCoupling α ℏ c m : ℝ)
    (hEntCoupling : 0 ≤ entropicCoupling) (hα : 0 ≤ α) (hh : 0 ≤ ℏ) (hc : 0 ≤ c) :
    0 ≤ entropicBackreactionScale entropicCoupling α ℏ c m :=
  entropicBackreactionScale_nonneg entropicCoupling α ℏ c m hEntCoupling hα hh hc
