...
/-- Angular-frequency dimension [ω] = T^-1. -/
def dimAngularFrequency : Dimension := Dimension.inv dimTime

/-- Speed dimension [c] = L T^-1. -/
def dimSpeed : Dimension := Dimension.div dimLength dimTime

/-- QCF bridge mass dimension [ħ_τ ω_τ / c^2]. -/
def dimQCFMassFromTauFrequency : Dimension :=
  Dimension.div (Dimension.mul dimHbar dimAngularFrequency) (Dimension.pow dimSpeed 2)
...
/-- QCF bridge dimensional identity from artifact extraction:
    [ħ_τ ω_τ / c^2] = [M]. -/
theorem dim_qcf_mass_from_tau_frequency_eq_mass :
    dimQCFMassFromTauFrequency = dimMass := by
  ext <;> simp [dimQCFMassFromTauFrequency, dimAngularFrequency, dimSpeed,
    dimHbar, dimAction, dimLength, dimTime, dimMass,
    Dimension.div, Dimension.mul, Dimension.inv, Dimension.pow]
