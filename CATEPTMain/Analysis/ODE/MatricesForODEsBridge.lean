...
namespace CATEPTMain.Analysis.ODE.Matrices
...
/-- **norm_sq_mtx_le**: operator norm bound via matrix entries.
    ...
    `‖A‖ ≤ n · max_{i,j} |A i j|`. -/
axiom afp_norm_sq_mtx_le {n : ℕ} [Norm (SqMtx n)] (A : SqMtx n) :
  ∃ C : ℝ, ‖A‖ ≤ C
...
axiom afp_norm_exp_mtx_le {n : ℕ} [Norm (SqMtx n)] (A : SqMtx n) (t : ℝ) (ht : 0 ≤ t) :
...
theorem ns_galerkin_ode_flow_anchor {n : ℕ}
    (A : SqMtx n) (b : ℝ → Fin n → ℝ) (hb : Continuous b) (t₀ : ℝ) (x₀ : Fin n → ℝ) :
    ∃! x : ℝ → Fin n → ℝ,
      (∀ t, HasDerivAt x (A.mulVec (x t) + b t) t) ∧ x t₀ = x₀ :=
  afp_picard_lindeloef_affine A b hb t₀ x₀
...
theorem ns_galerkin_energy_bound_anchor {n : ℕ}
  [Norm (SqMtx n)]
  (A : SqMtx n) (t : ℝ) (ht : 0 ≤ t) :
    ‖mtxExp (t • A)‖ ≤ Real.exp (t * ‖A‖) :=
  afp_norm_exp_mtx_le A t ht

end CATEPTMain.Analysis.ODE.Matrices
