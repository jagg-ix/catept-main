...
| `RelativityGREinsteinSymbolicLemmas` | **MT-5** named symbolic lemmas factoring the canonical Minkowski Einstein-electrovacuum obligations: `canonical_einstein_field_equations_reduce` (proved by `rfl`, no `Lean.ofReduceBool` dependency) and `canonical_maxwell_residual_array_zero` (now proved by kernel `rfl` under `set_option maxRecDepth 8192`; `native_decide` and the `Lean.ofReduceBool` axiom dependency have been removed after the Gravitas `simplifyN`/`symDiffN` totalization, so the axiom set reduces to the standard `[propext, Classical.choice, Quot.sound]` trio) |
...
| `RelativityGRWitnessFreeFamilyCertificate` | **MT-1 completion (fully parameterized)** `general_flat_electrovacuum_family` / `general_flat_electrovacuum_family_stress_conserved`: strictly generalizes the canonical-payload `IsFlatElectrovacuumFamily` constructor to arbitrary `(A, μ₀, Λ)` admitting `(hμ₀, hMaxwell, hFaraday)`. **Target 7** typed parameterized GR family certificate bundle (`ParameterizedGRFamilyCertificate`, `canonicalParameterizedGRFamilyCertificate`, `parameterizedGRFamilyCertificate_exists`) aggregating `hodgeStarEM_involutive_for_minkowski_family` (∀ F), `flat_constant_stress_conserved_for_all_constant_T` (∀ T), and `einstein_residual_zero_for_vacuum_family` (∀ Λ) into a single typed `Prop` with three real fields — closes the claim that the GR certificate covers a family of admissible GR data, not just one canonical instance |
| `RelativityGRWitnessFreeStressConservation` | **MT-2** conditional witness-free Maxwell-to-stress conservation (`maxwell_implies_stress_conservation_minkowski_witness_free`, `canonical_maxwell_implies_stress_conservation_witness_free`): takes `(hμ₀, hFaraday, hMaxwell)` and discharges conservation with no explicit stress-identification premise via MT-1 + MT-3 packaging. **MT-6** unconditional witness-free conservation (`IsNamedCanonicalElectrovacuumStress`, `named_canonical_electrovacuum_stress_conserved`, `canonical_named_canonical_electrovacuum_stress_conserved`): no Maxwell premise, no external witnesses; closes the witness-free GR Maxwell→conservation path at the named-canonical surface |
