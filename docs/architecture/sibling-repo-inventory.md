...
**Decision (2026-04-26): consolidate now.**

Execution rule:
1. `catept-domain-geometry` is canonical for QUAT going forward.
2. `catept-domain-core` is canonical for MTN going forward.
3. `catept-domain-quat` and `catept-domain-mtn` are legacy mirrors only
   (freeze for new feature work; keep as compatibility references).
...
## Resolved maintainer decisions (2026-04-26)

1. **Visibility policy for Class B/C/D:** keep all new domain/result siblings
   private by default.
2. **Thin-repo consolidation:** execute now (`catept-domain-quat` →
   `catept-domain-geometry`, `catept-domain-mtn` → `catept-domain-core`).
