...
/-! ## WP11: Turing-equivalence and DSL bridge (paper label aliases) -/

section WP11TuringDSLAliases

/-- paper4_eq_WP11_full_abstraction: Operational ≡ denotational equivalence on value observations. -/
theorem paper4_eq_WP11_full_abstraction (t u : LTerm) :
    OperationalEq t u ↔ DenotationalEq t u :=
  fullAbstraction_core t u

/-- paper4_eq_WP11_preservation: Type is preserved by one-step reduction. -/
theorem paper4_eq_WP11_preservation
    {Γ Δ : Context} {t t' : LTerm} {A : LType}
    (ht : HasType Γ Δ t A) (hs : Step t t') :
    HasType Γ Δ t' A :=
  preservation ht hs

/-- paper4_eq_WP11_progress: Closed well-typed terms are either values or can step. -/
theorem paper4_eq_WP11_progress {t : LTerm} {A : LType}
    (ht : HasType [] [] t A) :
    Value t ∨ ∃ t', Step t t' :=
  progress_closed ht

/-- paper4_eq_WP11_type_safety: Closed well-typed terms never get stuck under multi-step reduction. -/
theorem paper4_eq_WP11_type_safety {t t' : LTerm} {A : LType}
    (ht : HasType [] [] t A)
    (hstar : Relation.ReflTransGen Step t t') :
    Value t' ∨ ∃ u, Step t' u :=
  typeSafety_closed ht hstar

/-- paper4_eq_WP11_halting_invariance: Monitoring labels do not alter the halting observable. -/
theorem paper4_eq_WP11_halting_invariance :
    qtm_halting_measurement_invariance_spec :=
  qtm_halting_measurement_invariance

/-- paper4_eq_WP11_obligation_closure: Every tracked phase obligation has a Lean proof. -/
theorem paper4_eq_WP11_obligation_closure : ∀ id : ObligationId, obligationProp id :=
  obligationProp_proved

/-- paper4_eq_WP11_traceability: Artifact bindings are formally traceable to proved theorems. -/
theorem paper4_eq_WP11_traceability
    (artifactKey : String) (op : CommandType) (node : IRNode) (payload : RuntimePayload)
    (id : ObligationId)
    (hmem : id ∈ (mkArtifactBinding artifactKey op node payload).obligations) :
    obligationProp id :=
  artifact_binding_traceable artifactKey op node payload id hmem

end WP11TuringDSLAliases
