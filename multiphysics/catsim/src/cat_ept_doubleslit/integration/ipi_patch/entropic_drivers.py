...
# NOTE: i-PI is an optional dependency for this repository.
# Keep this module importable even when i-PI is not installed (e.g. CI, local
# usage focused on the double-slit pipeline). The driver entrypoints will raise
# a clear error if invoked without i-PI.
try:
    from ipi.utils.messages import Message  # type: ignore
    from ipi.interfaces.sockets import InterfaceSocket  # type: ignore
except Exception as e:  # pragma: no cover
    Message = None  # type: ignore
    InterfaceSocket = None  # type: ignore
    _IPI_IMPORT_ERROR = e
else:  # pragma: no cover
    _IPI_IMPORT_ERROR = None

from ...numerics.cfl_clock import CFLClock, dtau_from_dt
...
class EntropicDriver:
    ...
    def __init__(
        ...
        unix_socket=None,
        *,
        # CFL/step-control parameters (optional):
        cfl_dx_m=None,
        cfl_amax_m_s=None,
        cfl_max=0.9,
        alpha_scheme=2.0,
        ...
        """
        if InterfaceSocket is None:  # pragma: no cover
            raise ImportError(
                "i-PI is required to run entropic drivers. "
                "Install it (e.g. `pip install ipi`) and retry."
            ) from _IPI_IMPORT_ERROR
        ...
        # A small, self-contained step-control layer.
        # In i-PI workflows the *server* selects dt, but drivers can expose
        # recommended guards via `extras` for logging and/or custom motion.
        self.clock = CFLClock(
            dx=cfl_dx_m,
            a_max_default=cfl_amax_m_s,
            cfl_max=cfl_max,
    ...
    def run(self):
        ...
        while True:
            ...
            if Message is None:  # pragma: no cover
                raise RuntimeError("i-PI message constants unavailable")
            ...
            if msg == Message.STATUS:
                ...
                # --- CFL / dissipation guard suggestions ---
                # i-PI chooses dt on the server side. Here we expose a *recommended*
                # upper bound for dt (coordinate time) and the corresponding
                # entropic-time step d_tau = lambda_eff * dt.
                extras = self._attach_step_suggestions(extras, natoms=self.natoms)
    ...
    def _attach_step_suggestions(self, extras, *, natoms: int):
        """Attach dt/dtau recommendation fields to `extras`.

        i-PI itself may ignore unknown extras, but they are useful for:
        - logging & post-analysis
        - custom i-PI motion classes (like the entropic Langevin patch)
        """
        if extras is None:
            extras = {}

        lam = extras.get("lambda")
        if lam is None:
            return extras
        ...
        dt_suggest_opt = self.clock.suggest_dt(lambda_max=lambda_max)
        if dt_suggest_opt is None:
            return extras
        dt_suggest = float(dt_suggest_opt)

        dtau_opt = self.clock.suggest_dtau(lambda_eff=lambda_eff, lambda_max=lambda_max)
        dtau_suggest = float(dtau_opt) if dtau_opt is not None else float(dtau_from_dt(dt_suggest, lambda_eff))
        courant = float(self.clock.courant_number(dt_suggest))

        # Follow the convention used elsewhere in this module: extras are
        # lists sized natoms.
        extras["dt_suggest"] = [dt_suggest] * natoms
        extras["dtau_suggest"] = [dtau_suggest] * natoms
        extras["lambda_max"] = [lambda_max] * natoms
        extras["courant"] = [courant] * natoms
        extras["alpha_scheme"] = [float(self.clock.alpha_scheme)] * natoms
        return extras
