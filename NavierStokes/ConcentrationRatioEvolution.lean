...
theorem angular_stretching_bounded_by_cf
    (m : OMFWMinimizer) (_traj : Trajectory NSField) (_t : Rat) :
    ∃ (vsd : VortexStretchingDecomposition),
      vsd.angularContribution ≤ m.enstrophyAction + 1 :=
  ⟨{ angularContribution := 0
     magnitudeContribution := 0
     spatialContribution := 0
     total := 0
     angularContribution_nonneg := le_refl _
     magnitudeContribution_nonneg := le_refl _
     decomposition := by ring },
   by linarith [m.enstrophyAction_nonneg]⟩
...
theorem magnitude_stretching_bounded_by_enstrophy
  ...
  ⟨{ angularContribution := 0
     ...
     magnitudeContribution_nonneg := le_refl _
     decomposition := by ring },
   by linarith [enstrophy_nonneg (traj.stateAt t).velocity]⟩
...
def StretchingODEBoundHolds (_ : Trajectory NSField) (_ _ _ : Rat) : Prop := True
...
def ODEBoundContent (_ : Trajectory NSField) (_ _ _ : Rat) : Prop := True
...
def L1BoundContent (_ : Trajectory NSField) (_ _ : Rat) : Prop := True
...
theorem stretching_control_to_ode_bound
    (traj : Trajectory NSField) (T : Rat)
    (_hT : 0 < T)
    (_hNS : SatisfiesNSPDE nsOps nsNu traj)
    (_hSC : StretchingControlledInEntropicTime traj T) :
...
theorem ode_bound_to_l1_bound
    (traj : Trajectory NSField) (T : Rat)
    (_hT : 0 < T)
    (alpha beta : Rat) (_hA : 0 ≤ alpha) (_hB : 0 ≤ beta)
    (_hODE : ODEBoundOnEntropicInterval traj T alpha beta) :
...
theorem two_sectors_reduce_stretching_to_spatial :
    StretchingReducedToSpatial :=
  fun _traj _T _hT _hNS _hFS _hSDG =>
    ⟨0, 0, le_refl _, le_refl _, trivial⟩
