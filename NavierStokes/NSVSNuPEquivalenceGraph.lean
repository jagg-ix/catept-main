...
def vsnupFormCatalog : List VSNuPFormCatalogEntry :=
  ...
  , { id := .f23, symbol := "realNoetherToSliceVS_global_contract", formType := "theorem_root_contract_witness", bridgeLayer := "thermodynamic_root_contract", scope := "global", equivalenceKey := "root_contract_witness", equivalenceText := "Theorem witness of RealNoetherToSliceVSContract; derived from SA-G4 transport and equivalent in quantifier/inequality shape to canonical VSLeNuPAllTrajProp", sourceFile := "ThermodynamicRegularityBridge.lean", sourceLine := 166 } ]
