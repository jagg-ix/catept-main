...
axiom millennium_D_periodic_breakdown_counterexample_axiom
    {X : Type u}
    (ops : FieldOps X)
...
/-- Contract anchor theorem for D-path. This theorem is currently discharged via
    the explicit counterexample axiom above. -/
theorem millennium_D_periodic_breakdown_counterexample
    {X : Type u}
    (ops : FieldOps X)
    (spaces : FunctionSpaceAssumptions X)
    (nu : Rat) :
