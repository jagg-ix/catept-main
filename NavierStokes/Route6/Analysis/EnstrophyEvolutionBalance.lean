...
/-- The enstrophy rate: dΩ/dt for a trajectory at time t.
    For smooth NS solutions: dΩ/dt = -2ν·P + 2·VS.

    This is the pointwise-in-time identity, distinct from the energy rate
    dE/dt = -ν·Ω (which involves one fewer derivative). -/
axiom enstrophyRate : Trajectory NSField → Rat → Rat
...
/-- Viscous diffusion contribution to the enstrophy rate: 2∫ω·(ν∆ω) dx. -/
axiom enstrophyDiffusionContribution : Trajectory NSField → Rat → Rat

/-- Advective transport contribution to the enstrophy rate: -2∫ω·((u·∇)ω) dx. -/
axiom enstrophyTransportContribution : Trajectory NSField → Rat → Rat

/-- Step 1: Enstrophy rate splits into diffusion + transport + stretching. -/
axiom enstrophyRateDecomposition
...
/-- Step 2: Viscous diffusion contribution = -2ν·P (inner product with Laplacian). -/
axiom enstrophyDiffusionIsPalinstrophy
    (traj : Trajectory NSField) (t : Rat)
    (hNS : SatisfiesNSPDE nsOps nsNu traj)
    (hFS : RespectsFunctionSpaces nsSpacesR3 traj) :
    enstrophyDiffusionContribution traj t =
...
/-- Step 3: Advective transport vanishes by incompressibility (∇·u = 0). -/
axiom enstrophyTransportVanishes
    (traj : Trajectory NSField) (t : Rat)
    (hNS : SatisfiesNSPDE nsOps nsNu traj)
    (hFS : RespectsFunctionSpaces nsSpacesR3 traj) :
    enstrophyTransportContribution traj t = 0
...
axiom ladyzhenskayaConstant : Rat
axiom ladyzhenskayaConstant_pos : 0 < ladyzhenskayaConstant
...
/-- Product-form vortex stretching bound with named constant (corrected 3D form).
    ...
    Derivation: |VS| = |∫ωᵢωⱼ∂ⱼuᵢ| ≤ ‖ω‖²_{L³}·‖∇u‖_{L³}
    By Sobolev: ‖ω‖_{L³} ≤ C·‖ω‖_{L²}^{1/2}·‖∇ω‖_{L²}^{1/2} = C·Ω^{1/4}·P^{1/4}
    and ‖∇u‖_{L³} ≤ C·‖ω‖_{L²}^{1/4}·P^{1/4} = C·Ω^{1/8}·P^{1/4}
    Combining: |VS| ≤ C·Ω^{3/4}·P^{3/4}. -/
...
/-- Sub-axiom: vortex stretching integral is nonneg.
    Physically: VS = ∫ ωᵢ Sᵢⱼ ωⱼ ≥ 0 when strain aligns with vorticity
    in the principal stretching direction. This is a modeling convention:
    the absolute value |VS| is used in the G-N bound. -/
...
theorem subcritical_enstrophy_implies_stretching_dominated
  ...
  have hDiv := hFS.2.2 t
  have hPoinc := poincare_spectral_gap _ hDiv
...
/-- Opaque function: the time-integrated normalized stretching ∫₀^{τ_max} VS/Ω dτ.
    This is the key quantity whose boundedness implies the palinstrophy ratio
    (and hence BKM) is finite via the enstrophy budget identity. -/
axiom integratedNormalizedStretching : Trajectory NSField → Rat → Rat
...
/-- Sub-axiom 1: Enstrophy budget as a DIRECT inequality.
    Integrating the entropic enstrophy evolution dΩ/dτ = -2ℏ(P/Ω) + 2(ℏ/ν)(VS/Ω)
    over [0, τ_max] and using Ω(τ_max) ≥ 0:
      2ℏ · ∫P/Ω dτ ≤ Ω(0) + 2(ℏ/ν) · ∫VS/Ω dτ
    This is a RELATIONAL inequality — no existential bounds. It directly
    connects the palinstrophy ratio integral to the initial enstrophy and
    the stretching integral. -/
...
/-- Spatial direction gradient control implies the enstrophy budget
    ...
    Axiomatized: the spatial direction gradient conjecture controls
    the integrated normalized stretching ∫VS/Ω dτ, which then gives
    the spectral concentration bound via the enstrophy budget identity. -/
axiom spatial_to_budget_to_spectral :
    enstrophyEvolutionUnifiesReformulations
