...
/-! ## WP10: Kuchar six-problem status hardening (paper label aliases) -/

section WP10KucharAliases

/-- paper4_eq_WP10_step_valid: One Kuchar step preserves non-negativity of all scores. -/
theorem paper4_eq_WP10_step_valid (δ : Rat) (hδ : 0 < δ)
    (s : KucharConstructiveState) (hv : KucharConstructiveValid s) :
    KucharConstructiveValid (kucharStep δ hδ s) :=
  kucharStep_valid δ hδ s hv

/-- paper4_eq_WP10_clock_monotone: Kuchar clock advances strictly with each step. -/
theorem paper4_eq_WP10_clock_monotone (δ : Rat) (hδ : 0 < δ)
    (s : KucharConstructiveState) :
    s.clock < (kucharStep δ hδ s).clock :=
  kucharStep_clock_monotone δ hδ s

/-- paper4_eq_WP10_iterate_valid: N steps from a valid state remains valid. -/
theorem paper4_eq_WP10_iterate_valid (δ : Rat) (hδ : 0 < δ) (N : Nat)
    (s : KucharConstructiveState) (hv : KucharConstructiveValid s) :
    KucharConstructiveValid ((kucharStep δ hδ)^[N] s) :=
  kucharIterate_valid δ hδ N s hv

/-- paper4_eq_WP10_constructive_complete: Strictly positive scores → KucharComplete. -/
theorem paper4_eq_WP10_constructive_complete (s : KucharConstructiveState)
    (hs : KucharConstructiveSolved s) :
    KucharComplete {
      problem_of_time         := 0 < s.s1
      problem_of_observables  := 0 < s.s2
...
/-- paper4_eq_WP10_one_step_solves: One step from the zero state solves all six problems.
    This is the "hardened" status: a single δ-injection closes all Kuchar problems
    in the constructive numeric model. -/
theorem paper4_eq_WP10_one_step_solves (δ : Rat) (hδ : 0 < δ) :
    KucharConstructiveSolved
      (kucharStep δ hδ { s1 := 0, s2 := 0, s3 := 0, s4 := 0, s5 := 0, s6 := 0, clock := 0 }) := by
  simp [KucharConstructiveSolved, kucharStep, hδ]

/-- paper4_eq_WP10_qft_gr_core: UV renorm + BRST + Kuchar form a consistent stack. -/
theorem paper4_eq_WP10_qft_gr_core
    (s : RenormState) (b : BRSTState) (hs : UvAdmissible s) :
    UvAdmissible (renormStep s) ∧
    brst (brst b) = { gaugeField := 0, ghost := 0, antighost := 0 } :=
  deep_qft_gr_core_closures s b hs

end WP10KucharAliases
