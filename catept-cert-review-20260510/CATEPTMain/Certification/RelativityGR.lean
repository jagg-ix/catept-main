...
/-- Convert rank-2 tensor indices from whatever they are to contravariant. -/
private def toContravariant (gCov gInv t : Mat) (idx1 idx2 : IndexKind) : Mat :=
  ...
  | true, false => matBuild n (fun i j => sumN n (fun k => simplify (.mul (matGet gInv j k) (matGet t i k))))
  | false, true => matBuild n (fun i j => sumN n (fun k => simplify (.mul (matGet gInv i k) (matGet t k j))))

/-- Real covariant divergence operator for stress-energy tensors.
    ∇_μ T^{μν} = ∂_μ T^{μν} + Γ^μ_{μρ} T^{ρν} + Γ^ν_{μρ} T^{μρ}
    Implemented via exact AFP semantic-mapping for the Christoffel corrections. -/
def covariantDivergenceStressEnergy (g : MetricTensor) (T : StressEnergyTensor) : Array Gravitas.Expr :=
  let n := g.dim
  let tCon := toContravariant g.covariantMatrix g.inverseMatrix T.components T.idx1 T.idx2
  let coords := g.coords
  let Γ := ChristoffelSymbols.computeMixed g.covariantMatrix g.inverseMatrix coords
  let getΓ := fun lam μ ν => ChristoffelSymbols.getComp n Γ lam μ ν
  -- ∇_μ T^{μν}
...
/-- Canonical GR Einstein certificate (CERT-UP-005 Stage B).
    ...
    * `divergence_dimension` follows from `covariantDivergenceStressEnergy`
      returning `Array.ofFn (n := g.dim) ...`, whose `.size` is `g.dim` by
      `Array.size_ofFn` (kernel-checkable).
...
def canonical_gr_einstein : GRCATEPTEinsteinCertificate where
  ...
  divergence_dimension                  := by
    simp [covariantDivergenceStressEnergy, gravitasMinkowski]
