...
/-- In the current reduced-carrier stage (`enstrophy := 0`), full enstrophy
    alignment to physical observables is inconsistent with the nontrivial
    Fourier witness imported by `interpretAsFourier_nontrivial`. -/
theorem enstrophy_alignment_inconsistent_with_obs_nontrivial
    (hEnsAlign : ∀ v : NSField,
      enstrophy v = physicalNSObservables.enstrophy v) :
    False := by
  rcases interpretAsFourier_nontrivial with ⟨v, hvFourier⟩
  have hPhysPos : 0 < physicalNSObservables.enstrophy v := by
    rw [physicalObs_enstrophy_fourier_id v]
    exact hvFourier
  have hPhysZero : physicalNSObservables.enstrophy v = 0 := by
    calc
      physicalNSObservables.enstrophy v = enstrophy v := by
        symm
        exact hEnsAlign v
      _ = 0 := by
        simp [enstrophy]
  rw [hPhysZero] at hPhysPos
  linarith

/-- Therefore full Stage-218 alignment is also inconsistent in the current model,
    until the reduced-carrier `enstrophy` placeholder is concretized. -/
theorem alignment_inconsistent_with_obs_nontrivial
    (hAlign : PhysicalMode0ObsAlignment) :
    False :=
  enstrophy_alignment_inconsistent_with_obs_nontrivial hAlign.2
...
theorem bkmPhysicalMode0_eq_obs_physical
  ...
  congr 1
  funext t
  exact hVortAlign (traj.stateAt t).velocity
...
theorem entropicProperTime_eq_obs_physical
  ...
  congr 1
  funext t
  exact hEnsAlign (traj.stateAt t).velocity
...
/-- Current-model blocker:
    a full carrier swap `enstrophy = EnstrophyPhysicalizedCandidate` is
    impossible while `enstrophy` is still the zero placeholder. -/
theorem enstrophy_candidate_swap_impossible_current_model :
    ¬ (∀ v : NSField, enstrophy v = EnstrophyPhysicalizedCandidate v) := by
  intro hSwap
  rcases enstrophyPhysicalizedCandidate_positive_witness with ⟨v, hvPos⟩
  have hZero : enstrophy v = 0 := by simp [enstrophy]
  have hCandZero : EnstrophyPhysicalizedCandidate v = 0 := by
    calc
      EnstrophyPhysicalizedCandidate v = enstrophy v := by symm; exact hSwap v
      _ = 0 := hZero
  rw [hCandZero] at hvPos
  linarith
...
/-- In the current reduced-carrier model (`enstrophy := 0`), integrated enstrophy
    is identically zero on every trajectory/horizon. -/
theorem integratedEnstrophy_eq_zero_current_model
    (traj : Trajectory NSField) (T : Rat) :
    integratedEnstrophy traj T = 0 := by
  unfold integratedEnstrophy NavierStokes.DiscreteKernel.discreteIntegral
  simp [enstrophy, Finset.sum_const_zero]

/-- Therefore the Stage-218 physical mode-0 BKM integral is identically zero in
    the current model. -/
theorem bkmPhysicalMode0_eq_zero_current_model
    (traj : Trajectory NSField) (T : Rat) :
    bkmVorticityIntegralPhysicalMode0 traj T = 0 := by
  rw [bkmVorticityIntegralPhysicalMode0_eq_integratedEnstrophy]
  exact integratedEnstrophy_eq_zero_current_model traj T

/-- Current-model impossibility gate:
    the physical mode-0 non-placeholder witness cannot be discharged while
    `enstrophy` remains the reduced-carrier zero placeholder. -/
theorem physicalMode0_nonplaceholder_impossible_current_model :
    ¬ PhysicalMode0NonPlaceholderWitness := by
  intro h
  rcases h with ⟨traj, T, _hT, hPos⟩
  have hZero : bkmVorticityIntegralPhysicalMode0 traj T = 0 :=
    bkmPhysicalMode0_eq_zero_current_model traj T
  rw [hZero] at hPos
  linarith
...
/-- Corollary: the strong Stage-218 bridge target is impossible in the current
    reduced-carrier model until enstrophy is concretized. -/
theorem bridge_target_linear_entropic_control_physicalMode0Strong_impossible_current_model :
    ¬ BridgeTargetLinearEntropicControlPhysicalMode0Strong := by
  intro hStrong
  exact physicalMode0_nonplaceholder_impossible_current_model hStrong.2
