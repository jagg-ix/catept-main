...
theorem bkmVorticityIntegralObs_agmon_eq_fourier
  ...
  have heq : bkmVorticityIntegralObs fourierNSObsInstance_agmon traj T =
    ...
    unfold bkmVorticityIntegralObs enstrophyFTraj palinstrophyFTraj
    congr 1; funext t
    show enstrophyF (interpretAsFourier (traj.stateAt t).velocity) +
         palinstrophyF (interpretAsFourier (traj.stateAt t).velocity) =
         enstrophyF (trajFieldAt (liftTrajToFourier traj) t) +
         palinstrophyF (trajFieldAt (liftTrajToFourier traj) t)
...
theorem palinstrophyIntegralObs_agmon_eq_fourier
    ...
    integratedPalinstrophyF (liftTrajToFourier traj) T := by
  unfold palinstrophyIntegralObs integratedPalinstrophyF palinstrophyFTraj
  congr 1; funext t
  show palinstrophyF (interpretAsFourier (traj.stateAt t).velocity) =
       palinstrophyF (trajFieldAt (liftTrajToFourier traj) t)
  rw [liftTrajToFourier_fieldAt]
...
/-- Semantic nontriviality of the palinstrophy channel in `interpretAsFourier`'s image.
    ...
    Epistemic status: `.openBridge` — the Fourier mode structure of `interpretAsFourier`
    produces non-zero palinstrophy for any field with non-trivial high-frequency content.
    Analogous to `interpretAsFourier_nontrivial` (which witnesses non-zero enstrophy). -/
axiom interpretAsFourier_palinstrophy_nontrivial :
...
def stage152Summary : String :=
  ...
  "New axioms: interpretAsFourier_palinstrophy_nontrivial (openBridge). " ++
