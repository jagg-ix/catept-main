...
|-------|--------|--------------|
| 272 | bare axiom `.openBridge` | Classical.choose (non-constructive) |
...
/-- **K41 via AV decorrelation**: derives the Stage 272 existential using the
    ...
    This is the **third independent derivation route** for `k41_ept_universality`:
      Stage 272: bare `.openBridge` axiom
...
/-- The AV scaffold reduces `k41_ept_universality` (`.openBridge`) to three
