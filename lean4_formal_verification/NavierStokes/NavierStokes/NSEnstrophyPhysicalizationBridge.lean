...
/-- **THEOREM**: global Parseval alignment is discharged in the current concrete
    carrier model by unfolding both sides. -/
theorem enstrophyGlobalParsevalAlignment_discharged :
    EnstrophyGlobalParsevalAlignment := by
  intro v
  simp [NavierStokes.Millennium.enstrophy,
    NavierStokes.ObservableInterface.interpretAsFourier,
    NavierStokes.FourierModel.enstrophyF]
