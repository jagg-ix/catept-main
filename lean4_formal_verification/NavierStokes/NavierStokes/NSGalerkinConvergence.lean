...
axiom galerkinSplitting_step_lipschitz
    ...
    (galerkinSplitting_constants basis ν E₀ hν hE₀).lipC / 2 * h *
...
/-- **Combined one-step recurrence** — promoted from axiom to THEOREM (Stage 182).
    ...
    Then `|A+B|² ≤ 2|A|² + 2|B|²` (parallelogram, `normSqC_add_le`), and
      - `2|A|² ≤ 2·(lipC/2·h)·|e|² = lipC·h·|e|²`    (SA1, O(h) stability)
...
theorem galerkinSplitting_one_step_recurrence
    ...
    coeffNormSq (coeffSub
      (viscStep basis ν h (convStep basis u))
      (uExact (t + h))) ≤
    (1 + (galerkinSplitting_constants basis ν E₀ hν hE₀).lipC * h) *
  ...
  have hstab :
      ...
      sc.lipC / 2 * h * coeffNormSq (coeffSub u (uExact t)) :=
  ...
  calc coeffNormSq (fun i =>
    ...
    _ ≤ 2 * (sc.lipC / 2 * h * coeffNormSq (coeffSub u (uExact t))) +
        2 * (sc.lteC / 2 * h ^ 2) :=
          add_le_add (mul_le_mul_of_nonneg_left hstab (by norm_num))
                     (mul_le_mul_of_nonneg_left hlte (by norm_num))
    _ = sc.lipC * h * coeffNormSq (coeffSub u (uExact t)) + sc.lteC * h ^ 2 := by ring
    _ ≤ (1 + sc.lipC * h) * coeffNormSq (coeffSub u (uExact t)) + sc.lteC * h ^ 2 := by
          nlinarith [hnn]
...
theorem galerkinSplitting_consistency {N : Nat} (basis : GalerkinBasis N)
  ...
  rw [hzero] at h1
  linarith [mul_zero (1 + (galerkinSplitting_constants basis ν E₀ hν hE₀).lipC * h)]
...
theorem galerkinSplitting_gronwall_recurrence
    ...
    ∀ n : Nat,
      eAt (n + 1) ≤ (1 + sc.lipC * traj.h) * eAt n + sc.lteC * traj.h ^ 2 := by
  intro sc eAt n
...
/-- **Galerkin splitting convergence certificate** — assembles all three ingredients.
    ...
    - `(1 + lipC·h)^n ≤ exp(lipC·T)` (bounded, since n·h ≤ T)
...
theorem galerkinSplitting_convergence_certificate
    ...
    ∀ n : Nat,
      ...
      (1 + sc.lipC * traj.h) ^ n * (↑n * (sc.lteC * traj.h ^ 2)) := by
  intro sc n
  ...
  -- Positivity
  have hL : 0 ≤ sc.lipC * traj.h :=
    mul_nonneg (le_of_lt sc.lipC_pos) (le_of_lt traj.hh)
  have hB : 0 ≤ sc.lteC * traj.h ^ 2 :=
    mul_nonneg (le_of_lt sc.lteC_pos) (sq_nonneg _)
  exact galerkinSplitting_error_bound
          (sc.lipC * traj.h) (sc.lteC * traj.h ^ 2) hL hB eAt he0 hrec n
