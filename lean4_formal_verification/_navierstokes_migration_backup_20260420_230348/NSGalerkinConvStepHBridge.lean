import NavierStokes.Galerkin.NSGalerkinCayleyNearIdentityBridge
...
/-! ## Cayley increment bound (Stage 197) -/

/-- **Cayley increment bound**: `|convStepH h u − u|² ≤ C_K · E₀² · h²`.

    From the Cayley equation `convStepH h u i − u i = (h/2)·B(u, w) i` where
    `w = convStepH h u + u`, squaring and summing:

      `|convStepH h u − u|²`
      `= (h/2)²·∑|B(u,w) i|²`        [CRat_smul_normSqC + Cayley eq]
      `≤ (h/2)²·C_K·|u|²·|w|²`       [galerkinConvDef_normSq_le]
      `≤ (h/2)²·C_K·E₀·(4·E₀)`       [|u|²≤E₀, |w|²≤4·E₀ by parallelogram]
      `= C_K · E₀² · h²`              [ring]

    No `h ≤ 1` constraint needed.  Used in Stage 197 to retire the 196A axiom. -/
theorem convStepH_increment_bound {N : Nat} (basis : GalerkinBasis N)
    (h E₀ : Rat) (_hh : 0 < h) (hE₀ : 0 ≤ E₀)
    (u : CoeffC N) (hu : coeffNormSq u ≤ E₀) :
    coeffNormSq (coeffSub (convStepH basis h u) u) ≤
    triadKernelBound (standardTriadK basis) * E₀ ^ 2 * h ^ 2 := by
  have hCK := triadKernelBound_nonneg (standardTriadK basis)
  -- |convStepH h u + u|² ≤ 4·E₀  (parallelogram + energy preservation)
  have hw : coeffNormSq (fun j => convStepH basis h u j + u j) ≤ 4 * E₀ := by
    have heq := convStepH_energy_preserving basis h u
    simp only [coeffNormSq] at heq hu ⊢
    calc ∑ i : Fin N, normSqC (convStepH basis h u i + u i)
        ≤ ∑ i : Fin N, (2 * normSqC (convStepH basis h u i) + 2 * normSqC (u i)) := by
          apply Finset.sum_le_sum; intro i _
          simp only [normSqC, CRat.re, CRat.im, Prod.fst_add, Prod.snd_add]
          nlinarith [sq_nonneg ((convStepH basis h u i).1 - (u i).1),
                     sq_nonneg ((convStepH basis h u i).2 - (u i).2)]
      _ = 2 * ∑ i : Fin N, normSqC (convStepH basis h u i) +
          2 * ∑ i : Fin N, normSqC (u i) := by
            simp only [Finset.sum_add_distrib, ← Finset.mul_sum]
      _ ≤ 4 * E₀ := by linarith
  -- |B(u,w)|² ≤ C_K·|u|²·|w|² ≤ C_K·E₀·(4·E₀)
  have hnn_w : (0 : Rat) ≤ coeffNormSq (fun j => convStepH basis h u j + u j) :=
    Finset.sum_nonneg (fun i _ => normSqC_nonneg _)
  have hB : coeffNormSq (galerkinConvDef (standardTriadK basis) u
        (fun j => convStepH basis h u j + u j)) ≤
      triadKernelBound (standardTriadK basis) * E₀ * (4 * E₀) :=
    calc coeffNormSq (galerkinConvDef (standardTriadK basis) u
            (fun j => convStepH basis h u j + u j))
        ≤ triadKernelBound (standardTriadK basis) * coeffNormSq u *
            coeffNormSq (fun j => convStepH basis h u j + u j) :=
            galerkinConvDef_normSq_le _ u _
      _ ≤ triadKernelBound (standardTriadK basis) * E₀ * (4 * E₀) :=
            calc triadKernelBound (standardTriadK basis) * coeffNormSq u *
                    coeffNormSq (fun j => convStepH basis h u j + u j)
                ≤ triadKernelBound (standardTriadK basis) * E₀ *
                    coeffNormSq (fun j => convStepH basis h u j + u j) := by
                      apply mul_le_mul_of_nonneg_right _ hnn_w
                      exact mul_le_mul_of_nonneg_left hu hCK
              _ ≤ triadKernelBound (standardTriadK basis) * E₀ * (4 * E₀) :=
                    mul_le_mul_of_nonneg_left hw (mul_nonneg hCK hE₀)
  -- Rewrite coeffNormSq (coeffSub ...) using the Cayley equation
  have hnorm_eq :
      coeffNormSq (coeffSub (convStepH basis h u) u) =
      (h / 2) ^ 2 * coeffNormSq (galerkinConvDef (standardTriadK basis) u
        (fun j => convStepH basis h u j + u j)) := by
    simp only [coeffNormSq, coeffSub]
  ...
  calc (h / 2) ^ 2 * coeffNormSq (galerkinConvDef (standardTriadK basis) u
          (fun j => convStepH basis h u j + u j))
      ≤ (h / 2) ^ 2 * (triadKernelBound (standardTriadK basis) * E₀ * (4 * E₀)) :=
          mul_le_mul_of_nonneg_left hB (sq_nonneg _)
    _ = triadKernelBound (standardTriadK basis) * E₀ ^ 2 * h ^ 2 := by ring

/-! ## Stage 198A: First-order Euler-jet remainder bound -/

/-- **Euler-jet remainder bound** (h^4 in squared ℓ² norm) — **THEOREM**.

    The convective step `convStepH h u` differs from the first-order Euler jet
    `u + h·B(u,u)` by at most `(1/4)·C_K²·E₀³·h⁴` in squared ℓ² norm:

      `|convStepH h u − (u + h·B(u,u))|²  ≤  (1/4)·C_K²·E₀³·h⁴`

    Key identity: the remainder equals exactly `(h/2)·B(u, convStepH h u − u)`:

      `convStepH h u i − u i − h·B(u,u) i  =  (h/2)·B(u, wu−u) i`

    where `wu = convStepH h u`.  This follows from the Cayley equation
    `wu − u = (h/2)·B(u, wu+u)` and right-linearity
    `B(u, wu+u) = B(u, wu−u) + 2·B(u,u)`.

    Bound chain:
      `‖rem‖² = (h/2)²·‖B(u, wu−u)‖²`          [CRat_smul_normSqC]
      `≤ (h/2)²·C_K·|u|²·|wu−u|²`               [galerkinConvDef_normSq_le]
      `≤ (h/2)²·C_K·E₀·(C_K·E₀²·h²)`            [|u|²≤E₀, convStepH_increment_bound]
      `= (1/4)·C_K²·E₀³·h⁴`                      [ring]

    **Net: 0 new axioms** (uses only Stage 197 convStepH_increment_bound). -/
theorem convStepH_first_order_remainder_sq {N : Nat} (basis : GalerkinBasis N)
    (h E₀ : Rat) (hh : 0 < h) (hE₀ : 0 ≤ E₀)
    (u : CoeffC N) (hu : coeffNormSq u ≤ E₀) :
    coeffNormSq (fun i =>
      convStepH basis h u i - u i -
      CRat.smul h (galerkinConvection basis u u i)) ≤
    (1 / 4) * triadKernelBound (standardTriadK basis) ^ 2 * E₀ ^ 3 * h ^ 4 := by
  have hCK := triadKernelBound_nonneg (standardTriadK basis)
  -- Key identity: remainder = (h/2)·B(u, wu − u)
  have hrem : ∀ i : Fin N,
      convStepH basis h u i - u i -
      CRat.smul h (galerkinConvection basis u u i) =
      CRat.smul (h / 2)
        (galerkinConvection basis u (fun j => convStepH basis h u j - u j) i) := by
    intro i
    have hcay := convStepH_cayley_eq basis h u i
    -- B(u, wu+u) = B(u, (wu−u) + 2u) by right-linearity
    have hfun : (fun j => convStepH basis h u j + u j) =
        (fun j => (convStepH basis h u j - u j) + CRat.smul 2 (u j)) :=
      funext (fun j => Prod.ext
        (by simp only [CRat.smul, CRat.re, Prod.fst_add, Prod.fst_sub]; ring)
        (by simp only [CRat.smul, CRat.im, Prod.snd_add, Prod.snd_sub]; ring))
    have hsplit : galerkinConvection basis u (fun j => convStepH basis h u j + u j) i =
        galerkinConvection basis u (fun j => convStepH basis h u j - u j) i +
        galerkinConvection basis u (fun j => CRat.smul 2 (u j)) i := by
      rw [hfun]
      exact congr_fun (galerkinConvection_add_right_from_def basis u
        (fun j => convStepH basis h u j - u j) (fun j => CRat.smul 2 (u j))) i
    have hsmul2 : galerkinConvection basis u (fun j => CRat.smul 2 (u j)) i =
        CRat.smul 2 (galerkinConvection basis u u i) :=
      congr_fun (galerkinConvection_smul_right_from_def basis u u 2) i
    rw [hcay, hsplit, hsmul2]
    apply Prod.ext
    · simp only [CRat.smul, CRat.re, Prod.fst_sub, Prod.fst_add]; ring
    · simp only [CRat.smul, CRat.im, Prod.snd_sub, Prod.snd_add]; ring
  -- ‖rem‖² = (h/2)²·‖B(u, wu−u)‖²
  have hnorm_eq :
      coeffNormSq (fun i =>
        convStepH basis h u i - u i -
        CRat.smul h (galerkinConvection basis u u i)) =
      (h / 2) ^ 2 * coeffNormSq (fun i =>
        galerkinConvection basis u (fun j => convStepH basis h u j - u j) i) := by
    simp only [coeffNormSq]
    simp_rw [hrem, CRat_smul_normSqC]
    rw [← Finset.mul_sum]
  rw [hnorm_eq]
  -- ‖B(u, wu−u)‖² ≤ C_K·|u|²·|wu−u|²
  have hB_bound :
      coeffNormSq (fun i =>
        galerkinConvection basis u (fun j => convStepH basis h u j - u j) i) ≤
      triadKernelBound (standardTriadK basis) * coeffNormSq u *
        coeffNormSq (coeffSub (convStepH basis h u) u) := by
    have hkey := galerkinConvDef_normSq_le (standardTriadK basis) u
      (fun j => convStepH basis h u j - u j)
    have heq : ∀ i : Fin N,
        normSqC (galerkinConvection basis u (fun j => convStepH basis h u j - u j) i) =
        normSqC (galerkinConvDef (standardTriadK basis) u
          (fun j => convStepH basis h u j - u j) i) :=
      fun i => congr_arg normSqC
        (galerkinConvDef_is_galerkinConvection basis u
          (fun j => convStepH basis h u j - u j) i).symm
    have hcsub : coeffNormSq (fun j : Fin N => convStepH basis h u j - u j) =
        coeffNormSq (coeffSub (convStepH basis h u) u) := by
      simp only [coeffNormSq, coeffSub]
    simp only [coeffNormSq]
    simp_rw [heq]
  ...
  have hnn_inc : (0 : Rat) ≤ coeffNormSq (coeffSub (convStepH basis h u) u) :=
    Finset.sum_nonneg (fun i _ => normSqC_nonneg _)
  -- Chain
  calc (h / 2) ^ 2 * coeffNormSq (fun i =>
          galerkinConvection basis u (fun j => convStepH basis h u j - u j) i)
      ≤ (h / 2) ^ 2 * (triadKernelBound (standardTriadK basis) * coeffNormSq u *
          coeffNormSq (coeffSub (convStepH basis h u) u)) :=
          mul_le_mul_of_nonneg_left hB_bound (sq_nonneg _)
    _ ≤ (h / 2) ^ 2 * (triadKernelBound (standardTriadK basis) * E₀ *
          (triadKernelBound (standardTriadK basis) * E₀ ^ 2 * h ^ 2)) := by
          apply mul_le_mul_of_nonneg_left _ (sq_nonneg _)
          calc triadKernelBound (standardTriadK basis) * coeffNormSq u *
                  coeffNormSq (coeffSub (convStepH basis h u) u)
              ≤ triadKernelBound (standardTriadK basis) * E₀ *
                  coeffNormSq (coeffSub (convStepH basis h u) u) := by
                    apply mul_le_mul_of_nonneg_right _ hnn_inc
                    exact mul_le_mul_of_nonneg_left hu hCK
            _ ≤ triadKernelBound (standardTriadK basis) * E₀ *
                  (triadKernelBound (standardTriadK basis) * E₀ ^ 2 * h ^ 2) :=
                    mul_le_mul_of_nonneg_left hinc (mul_nonneg hCK hE₀)
    _ = (1 / 4) * triadKernelBound (standardTriadK basis) ^ 2 * E₀ ^ 3 * h ^ 4 := by ring

/-! ## Stage 198B: Viscous-step first-order remainder bound -/

/-- **Viscous-step Euler-jet remainder** (h^4 in squared ℓ² norm) — **THEOREM**.

    The implicit-Euler viscous step `viscStep ν h u` differs from the first-order
    Euler jet `u + h · viscousDamping ν u` by at most `ν^4 · galerkinN^8 · E₀ · h^4`:

      `|viscStep h u − (u + h·A(u))|²  ≤  ν^4 · galerkinN^8 · E₀ · h^4`

    Key algebraic identity (per mode): let `d_i = ν·h·|k_i|²`, `D_i = 1+d_i`:

      `viscStep_i − (u_i + h·A(u)_i) = (d_i²/D_i) · u_i`

    Proof chain:
      `normSqC(rem_i) = (d_i²/D_i)^2 · normSqC(u_i)`  [CRat_smul_normSqC]
      `≤ d_i^4 · normSqC(u_i)`                          [D_i ≥ 1]
      `≤ (ν·h·galerkinN^2)^4 · normSqC(u_i)`           [freq_le: k²_i ≤ galerkinN^2]
    Sum: `≤ ν^4 · galerkinN^8 · h^4 · E₀`               [|u|² ≤ E₀]
...
theorem viscStep_first_order_remainder_sq {N : Nat} (basis : GalerkinBasis N)
    ...
    coeffNormSq (fun i =>
      ...
      CRat.smul h (viscousDamping basis ν u i)) ≤
    ν ^ 4 * (galerkinN : Rat) ^ 8 * E₀ * h ^ 4 := by
  -- Per-mode identity: rem_i = CRat.smul (d_i²/D_i) (u_i)
  have hrem_eq : ∀ i : Fin N,
      viscStep basis ν h u i - u i -
      CRat.smul h (viscousDamping basis ν u i) =
      CRat.smul ((ν * h * waveVecMag2 (basis.wvec i)) ^ 2 /
                 (1 + ν * h * waveVecMag2 (basis.wvec i))) (u i) := by
    intro i
    have hD_ne : (1 : Rat) + ν * h * waveVecMag2 (basis.wvec i) ≠ 0 :=
      ne_of_gt (viscStep_denom_pos basis ν h hν hh i)
    apply Prod.ext
    · simp only [viscStep, viscousDamping, CRat.smul, Prod.fst_sub, CRat.re, CRat.im]
      field_simp [hD_ne]
      ring
    · simp only [viscStep, viscousDamping, CRat.smul, Prod.snd_sub, CRat.re, CRat.im]
      field_simp [hD_ne]
      ring
  -- normSqC of remainder = (d²/D)² · normSqC(u_i)
  have hnorm_eq :
      coeffNormSq (fun i =>
        viscStep basis ν h u i - u i -
        CRat.smul h (viscousDamping basis ν u i)) =
      ∑ i : Fin N,
        ((ν * h * waveVecMag2 (basis.wvec i)) ^ 2 /
         (1 + ν * h * waveVecMag2 (basis.wvec i))) ^ 2 * normSqC (u i) := by
    simp only [coeffNormSq]
    apply Finset.sum_congr rfl; intro i _
    rw [hrem_eq i, CRat_smul_normSqC]
  rw [hnorm_eq]
  -- Per-mode: (d²/D)² ≤ d^4 ≤ (ν·h·galerkinN^2)^4 = ν^4·galerkinN^8·h^4
  have hterm_bound : ∀ i : Fin N,
      ((ν * h * waveVecMag2 (basis.wvec i)) ^ 2 /
       (1 + ν * h * waveVecMag2 (basis.wvec i))) ^ 2 * normSqC (u i) ≤
      ν ^ 4 * (galerkinN : Rat) ^ 8 * h ^ 4 * normSqC (u i) := by
    intro i
    apply mul_le_mul_of_nonneg_right _ (normSqC_nonneg _)
    have hD_pos : (0 : Rat) < 1 + ν * h * waveVecMag2 (basis.wvec i) :=
      viscStep_denom_pos basis ν h hν hh i
    have hk2_nn := waveVecMag2_nonneg (basis.wvec i)
    have hd_nn : (0 : Rat) ≤ ν * h * waveVecMag2 (basis.wvec i) :=
      mul_nonneg (mul_pos hν hh).le hk2_nn
    -- (d²/D)² ≤ d^4 since D ≥ 1
    have h_frac_le : (ν * h * waveVecMag2 (basis.wvec i)) ^ 2 /
        (1 + ν * h * waveVecMag2 (basis.wvec i)) ≤
        (ν * h * waveVecMag2 (basis.wvec i)) ^ 2 :=
      div_le_self (sq_nonneg _) (by linarith)
    have h_frac_nn : (0 : Rat) ≤ (ν * h * waveVecMag2 (basis.wvec i)) ^ 2 /
        (1 + ν * h * waveVecMag2 (basis.wvec i)) :=
      div_nonneg (sq_nonneg _) hD_pos.le
    have hd4_ge : ((ν * h * waveVecMag2 (basis.wvec i)) ^ 2 /
                   (1 + ν * h * waveVecMag2 (basis.wvec i))) ^ 2 ≤
                  (ν * h * waveVecMag2 (basis.wvec i)) ^ 4 := by
      calc ((ν * h * waveVecMag2 (basis.wvec i)) ^ 2 /
              (1 + ν * h * waveVecMag2 (basis.wvec i))) ^ 2
          ≤ ((ν * h * waveVecMag2 (basis.wvec i)) ^ 2) ^ 2 :=
              pow_le_pow_left₀ h_frac_nn h_frac_le 2
        _ = (ν * h * waveVecMag2 (basis.wvec i)) ^ 4 := by ring
    -- d^4 ≤ (ν·h·galerkinN^2)^4 using freq_le
    have hk2_le : waveVecMag2 (basis.wvec i) ≤ (galerkinN : Rat) ^ 2 :=
      basis.freq_le i
    have hgN_nn : (0 : Rat) ≤ (galerkinN : Rat) ^ 2 := by positivity
    have hd4_gN : (ν * h * waveVecMag2 (basis.wvec i)) ^ 4 ≤
        ν ^ 4 * (galerkinN : Rat) ^ 8 * h ^ 4 := by
      have hmono : ν * h * waveVecMag2 (basis.wvec i) ≤ ν * h * (galerkinN : Rat) ^ 2 :=
        mul_le_mul_of_nonneg_left hk2_le (mul_pos hν hh).le
      calc (ν * h * waveVecMag2 (basis.wvec i)) ^ 4
          ≤ (ν * h * (galerkinN : Rat) ^ 2) ^ 4 :=
              pow_le_pow_left₀ (mul_nonneg (mul_pos hν hh).le hk2_nn) hmono 4
        _ = ν ^ 4 * (galerkinN : Rat) ^ 8 * h ^ 4 := by ring
    linarith
  calc ∑ i : Fin N,
          ((ν * h * waveVecMag2 (basis.wvec i)) ^ 2 /
           (1 + ν * h * waveVecMag2 (basis.wvec i))) ^ 2 * normSqC (u i)
      ≤ ∑ i : Fin N, ν ^ 4 * (galerkinN : Rat) ^ 8 * h ^ 4 * normSqC (u i) :=
          Finset.sum_le_sum (fun i _ => hterm_bound i)
    _ = ν ^ 4 * (galerkinN : Rat) ^ 8 * h ^ 4 * ∑ i : Fin N, normSqC (u i) := by
          rw [← Finset.mul_sum]
    _ ≤ ν ^ 4 * (galerkinN : Rat) ^ 8 * h ^ 4 * E₀ := by
          apply mul_le_mul_of_nonneg_left hu
          have hgN_nn : (0 : Rat) ≤ galerkinN := by norm_num [galerkinN]
          exact mul_nonneg
            (mul_nonneg (pow_nonneg hν.le 4) (pow_nonneg hgN_nn 8))
            (pow_nonneg hh.le 4)
    _ = ν ^ 4 * (galerkinN : Rat) ^ 8 * E₀ * h ^ 4 := by ring

def stage188Summary : String :=
  "Stage 188/198: NSGalerkinConvStepHBridge — h-parametric Cayley step near-identity. " ++
  ...
  "convStepH_increment_bound: THEOREM (Stage 197, |convStepH h u - u|² ≤ C_K·E₀²·h²). " ++
  "convStepH_first_order_remainder_sq: THEOREM (Stage 198A, |convStepH h u - (u+h·B(u,u))|² ≤ (1/4)·C_K²·E₀³·h⁴, 0 new axioms). " ++
  "viscStep_first_order_remainder_sq: THEOREM (Stage 198B, |viscStep h u - (u+h·A(u))|² ≤ ν⁴·galerkinN⁸·E₀·h⁴, 0 new axioms). " ++
  "Net: +1 def, +0 axioms, +11 theorems (1 private), 0 sorry."
