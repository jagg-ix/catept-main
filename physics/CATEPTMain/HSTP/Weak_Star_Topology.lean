...
namespace CATEPTMain.HSTP.Weak_Star_Topology
...
def HSTPWeakStarConv (Tseq : ℕ → HSTPOp) (T : HSTPOp) : Prop :=
  ∀ ρ : HSTPOp, CATEPTMain.HSTP.Trace_Class.IsTraceClass ρ →
    Filter.Tendsto
      (fun n => CATEPTMain.HSTP.Trace_Class.hstpTrace
        (CATEPTMain.HSTP.Strong_Operator_Topology.hstpOpComp ρ (Tseq n)))
      Filter.atTop
      (nhds (CATEPTMain.HSTP.Trace_Class.hstpTrace
        (CATEPTMain.HSTP.Strong_Operator_Topology.hstpOpComp ρ T)))
...
private axiom wot_eq_weakstar_law (Tseq : ℕ → HSTPOp) (T : HSTPOp) :
    ...
    CATEPTMain.HSTP.Weak_Operator_Topology.HSTPWeakConv Tseq T
...
axiom kaplansky_density (T : HSTPOp) (hBdd : hstpNorm T ≤ 1) :
    ∃ Tseq : ℕ → HSTPOp,
      (∀ n, CATEPTMain.HSTP.Compact_Operators.IsHSTPFiniteRank (Tseq n)) ∧
      (∀ n, hstpNorm (Tseq n) ≤ 1) ∧
      CATEPTMain.HSTP.Strong_Operator_Topology.HSTPStrongConv Tseq T

end CATEPTMain.HSTP.Weak_Star_Topology
