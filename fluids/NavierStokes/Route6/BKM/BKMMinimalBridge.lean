...
/-- Placeholder for the Beale-Kato-Majda time integral:
`int_0^T ||omega(.,t)||_(Linfty) dt`. -/
axiom bkmVorticityIntegral : Trajectory NSField -> Rat -> Rat

/-- Placeholder for entropic proper time at horizon `T`:
`tau_ent(T) = (nu/hbar) int_0^T ||grad u||^2 dt`. -/
axiom entropicProperTime : Trajectory NSField -> Rat -> Rat

/-- Opaque convergence predicate: the BKM vorticity integral
    ∫₀ᵀ ‖ω(·,t)‖_{L∞} dt genuinely converges as a Lebesgue integral.

    This is needed because `bkmVorticityIntegral : ... → Rat` cannot represent
    divergent integrals (Lean4 `Rat` has no infinity). Without this predicate,
    `∃ M : Rat, bkmVorticityIntegral traj T ≤ M` would be trivially true
    (take M = bkm value, apply le_refl).

    The old definition `def BKMIntegralFiniteAt := ∃ M, bkm ≤ M` was vacuously
    satisfiable. The fix wraps an opaque convergence predicate so BKM finiteness
    can only be established through axioms encoding genuine PDE content. -/
axiom BKMIntegralConverges : Trajectory NSField → Rat → Prop
...
/-- If the BKM vorticity integral is bounded above by some finite M,
    then the integral genuinely converges. This bridges the concrete
    `bkmVorticityIntegral traj T ≤ M` bound (computable in Rat) to the
    opaque convergence predicate `BKMIntegralConverges`.

    Without this axiom, proofs that establish a concrete upper bound on
    the integral value would have no way to conclude convergence. -/
axiom bkm_bounded_implies_converges
...
axiom minimal_bridge_to_globalVorticityControl
...
/-- Open transfer obligation in strongest-form language:
AQFT modular + entanglement first law + QRF transport imply
statewise global vorticity control. -/
axiom aqft_qrf_transfer_to_globalVorticityControl
    (pi : PathIntegralInterface NSField)
    (hStrong : AQFTQRFMinimalBridgeTheorem pi) :
    ∀ (st0 : State NSField),
      pi.PIWellPosed st0 ->
...
axiom fwRateFunctional : Trajectory NSField → Rat → Rat

/-- FW rate functional is non-negative (squared L² norm of PDE residual). -/
axiom fwRateFunctional_nonneg :
    ∀ (traj : Trajectory NSField) (T : Rat),
      0 < T → 0 ≤ fwRateFunctional traj T

/-- FW rate functional is zero iff trajectory solves NS exactly. -/
axiom fwRateFunctional_zero_iff_ns :
    ∀ (traj : Trajectory NSField) (T : Rat),
      0 < T →
      (fwRateFunctional traj T = 0 ↔ SatisfiesNSPDE nsOps nsNu traj)
...
theorem fw_coercivity_bridge_implies_precise_gap
  ...
  exact ⟨F, fun traj T hT hNS hFS =>
    hF traj T hT hNS hFS ((fwRateFunctional_zero_iff_ns traj T hT).mpr hNS)⟩
...
/-- The BKM proof decomposition is well-formed (all ingredients are standard PDE results). -/
axiom bkmProofDecompositionExists : BKMProofDecomposition
...
axiom gradientNormSquared : NSField → Rat

/-- Divergence correction term: the difference between enstrophy (‖curl u‖²)
    and gradient norm squared (‖∇u‖²). By Lagrange's vector identity in Fourier:
    |k × û|² = |k|²|û|² - (k · û)², so the correction is -Σ(k · û(k))².
    For divergence-free fields, k · û = 0 and the correction vanishes. -/
axiom enstrophyDivergenceCorrection : NSField → Rat

/-- Sub-axiom 1 (Lagrange identity): Enstrophy decomposes as gradient norm
    squared plus a divergence correction. In Fourier:
    Σ|k × û|² = Σ|k|²|û|² - Σ(k · û)² = gradientNormSquared - correction.
    Equivalently: enstrophy = gradientNormSquared + correction
    (where correction = -Σ(k · û)² ≤ 0 in general). -/
axiom enstrophyGradientDecomposition (v : NSField) :
    enstrophy v = gradientNormSquared v + enstrophyDivergenceCorrection v

/-- Sub-axiom 2 (Incompressibility cancellation): For NS trajectories,
    the velocity is divergence-free (∇ · u = 0), so k · û(k) = 0 in Fourier,
    making the correction term vanish identically.
    This is the key step where incompressibility enters. -/
axiom enstrophyDivFreeCorrection
    (traj : Trajectory NSField) (t : Rat)
    (hNS : SatisfiesNSPDE nsOps nsNu traj) :
    enstrophyDivergenceCorrection (traj.stateAt t).velocity = 0
...
/-- Integrated enstrophy over [0,T]: int_0^T ||omega(t)||^2_{L^2} dt. -/
axiom integratedEnstrophy : Trajectory NSField → Rat → Rat

/-- Reduced Planck constant (placeholder for dimensional consistency). -/
axiom hbar : Rat

/-- hbar > 0. -/
axiom hbar_pos : (0 : Rat) < hbar

/-- Entropic proper time via enstrophy (consequence of enstrophy-gradient identity):
    tau_ent(T) = (nu/hbar) int_0^T ||omega||^2 dt. -/
axiom entropicTimeViaEnstrophy :
    ∀ (traj : Trajectory NSField) (T : Rat),
      SatisfiesNSPDE nsOps nsNu traj →
...
axiom energyLinearInEntropicTime :
    ∀ (traj : Trajectory NSField) (T : Rat),
      SatisfiesNSPDE nsOps nsNu traj →
...
/-- The concentration ratio: ‖ω‖_{L∞} / ‖∇u‖²_{L²}.
    This is the natural BKM integrand in entropic time.
    The BKM integral reparametrizes as:
      ∫₀ᵀ ‖ω‖_{L∞} dt = (ℏ/ν) ∫₀^{τ_max} R(τ) dτ
    where R(τ) = ‖ω(τ)‖_{L∞} / ‖∇u(τ)‖² is dimensionless. -/
axiom concentrationRatio : Trajectory NSField → Rat → Rat

/-- Concentration ratio is non-negative. -/
axiom concentrationRatio_nonneg :
    ∀ (traj : Trajectory NSField) (t : Rat),
      0 ≤ concentrationRatio traj t
...
axiom bkmIntegralEntropicTimeReparametrization :
    ∀ (traj : Trajectory NSField) (T : Rat),
      0 < T →
      SatisfiesNSPDE nsOps nsNu traj →
      ∃ (intR : Rat),
...
/-- Opaque: Cameron-Martin weight exp(-S_I/ℏ) evaluated along a trajectory.
    In Rat arithmetic, the exponential is opaque; the structural content
    is that it remains bounded below by exp(-E₀/ℏ) > 0. -/
axiom cameronWeight : Trajectory NSField → Rat → Rat
...
/-- Placeholder for entropic-time integral of the concentration ratio:
    ∫₀^τ R(s) ds. -/
axiom entropicRatioIntegral : Trajectory NSField → Rat → Rat
...
axiom bkmIntegralReparametrizedByEntropicRatio :
    ∀ (traj : Trajectory NSField) (T : Rat),
      0 < T →
      SatisfiesNSPDE nsOps nsNu traj →
      RespectsFunctionSpaces nsSpacesR3 traj →
