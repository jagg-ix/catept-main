...
axiom helicity : Trajectory NSField → Rat → Rat

/-- Opaque helicity rate function dH/dt for a trajectory at time t.
    For NS: dH/dt = −2ν ∫ ω·(∇×ω) dx (integration by parts). -/
axiom helicityRateFunction : Trajectory NSField → Rat → Rat

/-- Helicity dissipation rate squared is bounded by C·Ω².
    Cauchy-Schwarz gives |dH/dt| ≤ 2ν·√Ω·√P; the opaque constant C
    absorbs the ν² and P/Ω factors (finite for regular solutions). -/
axiom helicityDissipationBound
...
/-- Euler helicity conservation: dissipation rate is O(ν).
    |dH/dt|² ≤ ν²·C·Ω² shows helicity rate → 0 as ν → 0
    (Kelvin-Helmholtz theorem in the inviscid limit). -/
axiom euler_helicity_viscous_bound
    (traj : Trajectory NSField) (t : Rat)
    (ht : 0 ≤ t) :
    ∃ (C : Rat), 0 < C ∧
      helicityRateFunction traj t * helicityRateFunction traj t ≤
