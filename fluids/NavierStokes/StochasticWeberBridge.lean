...
-- Stage 137: promoted to def (conservative lower bound: norm ≥ 0, value 0 satisfies nonneg)
noncomputable def inverseJacobianNorm (_flow : StochasticFlowMap) (_t : Rat) : Rat := 0

/-- The inverse Jacobian norm is nonneg. Stage 137: promoted to theorem. -/
theorem inverseJacobianNorm_nonneg :
...
/-- The stochastic action S_I accumulated along Cameron-weighted paths:
    S_I(T) = ℏ · τ_ent(T) = ℏ · (ν/ℏ) · ∫‖∇u‖² dt.
    Stage 132: concrete def — stochastic_action_is_entropic_time becomes rfl+rw. -/
noncomputable def stochasticActionFunctional (flow : StochasticFlowMap) (T : Rat) : Rat :=
  hbar * entropicProperTime flow.deterministicTrajectory T

/-- The stochastic action equals ℏ · τ_ent along the deterministic trajectory.
    Stage 132: promoted to theorem — rw [hMatch] closes the goal. -/
theorem stochastic_action_is_entropic_time
    (flow : StochasticFlowMap)
    ...
    (_hNu : flow.nu = nsNu) :
    stochasticActionFunctional flow _T = hbar * entropicProperTime traj _T := by
  unfold stochasticActionFunctional
  rw [hMatch]
