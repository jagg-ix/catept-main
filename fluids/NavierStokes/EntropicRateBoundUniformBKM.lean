...
/-- Cubic enstrophy integral ∫₀ᵀ Ω(t)³ dt.
    Concrete left Riemann sum over physical time with step 1/1000.
    Stage 116: replaces former opaque axiom — zero new axioms introduced. -/
noncomputable def integratedEnstrophyCube
...
/-- Cubic enstrophy integral is nonneg (Ω³ ≥ 0 since Ω ≥ 0). -/
theorem integratedEnstrophyCube_nonneg (traj : Trajectory NSField) (T : Rat) :
    0 ≤ integratedEnstrophyCube traj T := by
  unfold integratedEnstrophyCube
  apply NavierStokes.DiscreteKernel.discreteIntegral_nonneg
  intro t
  have hE := enstrophy_nonneg (traj.stateAt t).velocity
  exact mul_nonneg (mul_nonneg hE hE) hE
...
axiom pointwise_cap_bounds_cubic_integral :
    ∀ (traj : Trajectory NSField) (T : Rat) (OmegaMax : Rat),
    (∀ t : Rat, 0 ≤ t → t ≤ T →
...
/-- Time-integrated palinstrophy ∫₀ᵀ P(t) dt.
    Concrete left Riemann sum over physical time with step 1/1000.
    Stage 115: replaces former opaque axiom — zero new axioms introduced. -/
noncomputable def integratedPalinstrophy
    (traj : Trajectory NSField) (T : Rat) : Rat :=
  NavierStokes.DiscreteKernel.discreteIntegral
    (fun t => palinstrophy (traj.stateAt t).velocity) T

/-- Palinstrophy integral is nonneg (P ≥ 0). -/
theorem integratedPalinstrophy_nonneg (traj : Trajectory NSField) (T : Rat) :
    0 ≤ integratedPalinstrophy traj T := by
  unfold integratedPalinstrophy
  apply NavierStokes.DiscreteKernel.discreteIntegral_nonneg
  intro t; exact palinstrophy_nonneg (traj.stateAt t).velocity

/-- Zero horizon gives zero palinstrophy integral. -/
theorem integratedPalinstrophy_zero (traj : Trajectory NSField) :
    integratedPalinstrophy traj 0 = 0 := by
  unfold integratedPalinstrophy
  exact NavierStokes.DiscreteKernel.discreteIntegral_zero _
...
axiom omfw_action_plus_cubic_gap_implies_palinstrophy_bound :
    ∀ (traj : Trajectory NSField) (T : Rat) (Cdata : Rat),
    omCATFunctional traj T ≤ Cdata →
    integratedEnstrophyCube traj T ≤ Cdata →
...
theorem palinstrophy_bound_implies_stretching_control
    (traj : Trajectory NSField) (T : Rat)
    (_hPal : ∃ M : Rat, integratedPalinstrophy traj T ≤ M) :
    StretchingControlledInEntropicTime traj T :=
  ⟨0, 0, le_refl _, le_refl _, trivial⟩
