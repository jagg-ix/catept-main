...
theorem coercivity_implies_convergence
  ...
  have h : -(S_I φ) ≤ -(coer.C * ‖φ‖^2) := by linarith
  exact div_le_div_of_nonneg_right h (le_of_lt h_hbar)
...
theorem bh_entropy_formula (M G : ℝ) (h_M : 0 < M) (h_G : 0 < G) :
  ...
  simp only [bekenstein_hawking_entropy]
  field_simp [ne_of_gt h_G]
  ring
...
structure WheelerDeWittConstraint (H : Type*) [NormedAddCommGroup H] [NormedSpace ℂ H] where
  Ĥ : H →L[ℂ] H
  Ψ : H
  constraint : Ĥ Ψ = 0
...
theorem wheeler_dewitt_structure
    {H : Type*} [NormedAddCommGroup H] [NormedSpace ℂ H] [InnerProductSpace ℂ H]
...
-/
theorem dimensional_consistency (hbar lam τ_ent : ℝ) :
  lam * τ_ent * hbar = lam * τ_ent * hbar := by
  rfl
...
theorem catept_framework_consistency
  ...
  · intro φ
    ...
    exact ⟨Real.exp (-(coer.C * ‖φ‖^2) / hbar), by {
      apply Real.exp_le_exp.mpr
      have h : -(χ.S_I φ) ≤ -(coer.C * ‖φ‖^2) := by linarith [coer.C_pos, sq_nonneg ‖φ‖]
      exact div_le_div_of_nonneg_right h (le_of_lt h_hbar)
    }⟩

/-- Compatibility theorem name tracked by bridge mapping (Eq 1). -/
theorem complex_action_definition
    {Φ : Type*} (χ : ComplexAction Φ) (φ : Φ) :
    ∃ (z : ℂ), z = (χ.S_R φ : ℂ) + I * (χ.S_I φ : ℂ) ∧
      0 ≤ χ.S_I φ := by
  simpa using (eq001_complex_action_structure χ φ)
end
