...
Notation note:
- `κ` and `k_B` below are hyperbolic/thermal constants inherited from the
  theoremized Unruh/modular identities.
- They are distinct from the muon g-2 coefficient `entropicCoupling` used in
  `MuonGMinus2Bridge`.
-/
...
theorem imaginary_action_from_schmidt
    (schmidtK hbar : ℝ) (hK : 1 < schmidtK) (h_hbar : hbar ≠ 0) :
    ∃ S_i : ℝ, S_i = hbar * log schmidtK ∧ exp (-S_i / hbar) = 1 / schmidtK := by
  refine ⟨hbar * log schmidtK, rfl, ?_⟩
  have hK0 : 0 < schmidtK := by linarith
  rw [show -(hbar * log schmidtK) / hbar = -log schmidtK by field_simp [h_hbar]]
