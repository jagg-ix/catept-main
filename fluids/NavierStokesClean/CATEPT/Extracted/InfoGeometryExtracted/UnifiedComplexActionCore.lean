...
def phaseGravityThermalScore
  ...
  gravityCouplingFromInfo env r rCurv lambdaInfo miePhase * dtauEff + epsIm
...
def ringEnvFromExtractedEnv (env : ExtractedEnv) (newtonG : Real) : Atoms.RingEnv :=
  { c := env.c, newtonG := newtonG }

/-- Bridge wrapper to extracted atom `S0099.omegaLT_point`. -/
def omegaLT_point_from_atoms
    (env : ExtractedEnv) (newtonG : Real) (Jvec x : Atoms.Vec3) : Real :=
  Atoms.S0099.omegaLT_point (ringEnvFromExtractedEnv env newtonG) Jvec x

/-- Name-preserving equality for the atom bridge wrapper. -/
theorem omegaLT_point_from_atoms_eq
    (env : ExtractedEnv) (newtonG : Real) (Jvec x : Atoms.Vec3) :
    omegaLT_point_from_atoms env newtonG Jvec x =
      Atoms.S0099.omegaLT_point (ringEnvFromExtractedEnv env newtonG) Jvec x := rfl
