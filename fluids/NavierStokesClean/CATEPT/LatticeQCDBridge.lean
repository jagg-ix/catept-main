...
/-! ## WP09: Lattice-QCD continuum compatibility upgrade (paper label aliases) -/

section WP09ContinuumAliases

/-- paper4_eq_WP09_wilson_action: Wilson plaquette action definition. -/
theorem paper4_eq_WP09_wilson_action (L : LatticeWilsonData) :
    L.wilsonAction = ∑ i : Fin L.nPlaquettes, L.plaquetteAction i := rfl

/-- paper4_eq_WP09_boltzmann_pos: Boltzmann weight is strictly positive. -/
theorem paper4_eq_WP09_boltzmann_pos (L : LatticeWilsonData) :
    0 < L.boltzmannFactor :=
  L.boltzmannFactor_pos

/-- paper4_eq_WP09_boltzmann_le_one: Boltzmann weight bounded by 1 (damping). -/
theorem paper4_eq_WP09_boltzmann_le_one (L : LatticeWilsonData)
    (h_nonneg : 0 ≤ L.wilsonAction) :
    L.boltzmannFactor ≤ 1 :=
  L.boltzmannFactor_le_one h_nonneg

/-- paper4_eq_WP09_expectation_tendsto: Lattice expectation converges to continuum limit. -/
theorem paper4_eq_WP09_expectation_tendsto
    {Ls : ℕ → LatticeWilsonData}
    {hbar : ℝ} {hbar_pos : 0 < hbar}
    {hPlaquette_nonneg : ∀ n i, 0 ≤ (Ls n).plaquetteAction i}
    {J O : ∀ n, Fin (Ls n).nPlaquettes → ℂ}
    ...
    Filter.Tendsto
      (latticeSourceCoupledExpectationSeq Ls hbar hbar_pos hPlaquette_nonneg J O)
      Filter.atTop (nhds (C.unnormLimit / C.partitionLimit)) :=
  C.expectation_tendsto

/-- paper4_eq_WP09_W_tendsto: Lattice connected generating functional converges to W[J]. -/
theorem paper4_eq_WP09_W_tendsto
    {Ls : ℕ → LatticeWilsonData}
    {hbar : ℝ} {hbar_pos : 0 < hbar}
    {hPlaquette_nonneg : ∀ n i, 0 ≤ (Ls n).plaquetteAction i}
    {J O : ∀ n, Fin (Ls n).nPlaquettes → ℂ}
    ...
    Filter.Tendsto
      (latticeConnectedGeneratingFunctionalSeq Ls hbar hbar_pos hPlaquette_nonneg J)
      Filter.atTop (nhds (Complex.log C.partitionLimit)) :=
  C.connectedGeneratingFunctional_tendsto

/-- paper4_eq_WP09_pair_tendsto: Both expectation and W[J] converge simultaneously. -/
theorem paper4_eq_WP09_pair_tendsto
    {Ls : ℕ → LatticeWilsonData}
    {hbar : ℝ} {hbar_pos : 0 < hbar}
    {hPlaquette_nonneg : ∀ n i, 0 ≤ (Ls n).plaquetteAction i}
    {J O : ∀ n, Fin (Ls n).nPlaquettes → ℂ}
...
/-- paper4_eq_WP09_quadrature_kernel: Lattice quadrature equals certified discrete kernel. -/
theorem paper4_eq_WP09_quadrature_kernel (f : Rat → Rat) (T : Rat) :
    latticeQuadrature f T = NavierStokesClean.DiscreteKernel.discreteIntegral f T :=
  latticeQuadrature_eq_discreteKernel f T

end WP09ContinuumAliases
